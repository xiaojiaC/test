...
func main() {
	...
	go func() {
		...
		var client example.GreeterClient
		if *useGRPC {
		...
		for i := 0; i < *numRuns; i++ {
			...
			var proto string
			if *useGRPC {
				_, err = client.SayHello(ctx, &example.HelloRequest{Name: msg})
				proto = "GRPC"
			} else {
...
func (s greeter) SayHello(ctx context.Context, req *example.HelloRequest) (*example.HelloReply, error) {
